...
<nav x-data="{ open: false }" class="bg-dark-900/95 backdrop-blur-sm border-b border-dark-800 sticky top-0 z-50">
        ...
        <div class="flex justify-between h-16">
            <div class="flex items-center">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('home') }}" class="text-2xl font-bold text-white hover:text-brand-DEFAULT transition-colors">
                        HoodieShop
                    </a>
                </div>
            ...
            <div class="hidden sm:flex sm:items-center">
                <a href="{{ route('cart.index') }}" class="relative text-dark-300 hover:text-white transition-colors p-2">
                    ...
                    <span class="absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-white transform translate-x-1/2 -translate-y-1/2 bg-brand-DEFAULT rounded-full" x-data="{ count: {{ $this->cartItemCount() }} }" x-text="count" x-init="console.log(count)"></span>
                </a>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-dark-800">
        <div class="pt-2 pb-3 space-y-1">
            ...
            <a href="{{ route('cart.index') }}" class="block px-3 py-2 border-l-4 border-brand-DEFAULT text-base font-medium text-white bg-dark-700 focus:outline-none focus:border-brand-DEFAULT transition duration-150 ease-in-out">
                Warenkorb <span class="text-brand-DEFAULT text-sm ml-2" x-data="{ count: {{ $this->cartItemCount() }} }" x-text="count"></span>
            </a>
